...
def compute_cost_function(X, Y, theta, lambda_factor, temp_parameter):
    ...
    log_prob = np.log(H + 1e-15)

    cross_entropy_cost = -1 / n * np.sum(M * log_prob)

    regularization_cost = lambda_factor / 2 * np.sum(theta ** 2)
...
def run_gradient_descent_iteration(X, Y, theta, alpha, lambda_factor, temp_parameter):
    ...
    gradient = -1 / (temp_parameter * n) * (M - H) @ X + lambda_factor * theta
...
def compute_test_error_mod3(X, Y, theta, temp_parameter):
    ...
    predicted_y = np.argmax(probabilities, axis=0) % 3
